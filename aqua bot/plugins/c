...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        let res = await fetch(`https://api.betabotz.eu.org/api/primbon/ramalankeberuntungan?tanggal=${tanggal1}&bulan=${bulan1}&tahun=${tahun1}&apikey=${lann}`);
...
handler.help = ['ramalankeberuntungan']
handler.tags = ['fun']
handler.command = /^(ramalankeberuntungan)$/i
handler.group = true
