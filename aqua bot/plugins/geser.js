...
const { generateWAMessageContent, generateWAMessageFromContent, prepareWAMessageMedia, proto } = require('@whiskeysockets/baileys');

let handler = async (m, { usedPrefix, command, conn, args }) => {
  if (!text) throw ` Example:* ${usedPrefix}${command} Tobrut`;
  m.reply('Please wait...');
  ...
  try {
   ...
   let nem = await conn.getName(m.sender)
    ...
    shuffleArray(res);
    let limit = Math.min(5, res.length);
    ...
    for (let pus of images) {
      push.push({
        body: proto.Message.InteractiveMessage.Body.fromObject({
          text: `${limit} from PINTEREST SEARCH`
        }),
...
handler.help = handler.command = ['geser'];
